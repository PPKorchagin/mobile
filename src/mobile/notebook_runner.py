...
from mobile.project_paths import (
    ...
    DEFAULT_NB_STG_GEO_INTERVALS_EXECUTED_PATH,
    DEFAULT_NB_STG_GEO_INTERVALS_NOTEBOOK_PATH,
    DEFAULT_NB_STG_PERSON_EXECUTED_PATH,
    DEFAULT_NB_STG_PERSON_NOTEBOOK_PATH,
    DEFAULT_NB_STG_MSISDN_IMEI_EXECUTED_PATH,
    DEFAULT_NB_STG_MSISDN_IMEI_NOTEBOOK_PATH,
    DEFAULT_NB_STG_MSISDN_IMSI_OPERATOR_EXECUTED_PATH,
    DEFAULT_NB_STG_MSISDN_IMSI_OPERATOR_NOTEBOOK_PATH,
...
def run_nb_stg_event() -> None:
    if DEFAULT_NB_STG_EVENT_EXECUTED_PATH.exists():
        DEFAULT_NB_STG_EVENT_EXECUTED_PATH.unlink()
    run_notebook(
        source_notebook=DEFAULT_NB_STG_EVENT_NOTEBOOK_PATH,
        executed_notebook=DEFAULT_NB_STG_EVENT_EXECUTED_PATH,
    )


def run_nb_stg_bs() -> None:
    if DEFAULT_NB_STG_BS_EXECUTED_PATH.exists():
        DEFAULT_NB_STG_BS_EXECUTED_PATH.unlink()
    run_notebook(
...
def run_nb_stg_msisdn_imei() -> None:
    if DEFAULT_NB_STG_MSISDN_IMEI_EXECUTED_PATH.exists():
        DEFAULT_NB_STG_MSISDN_IMEI_EXECUTED_PATH.unlink()
    run_notebook(
        source_notebook=DEFAULT_NB_STG_MSISDN_IMEI_NOTEBOOK_PATH,
        executed_notebook=DEFAULT_NB_STG_MSISDN_IMEI_EXECUTED_PATH,
    )


def run_nb_stg_msisdn_imsi_operator() -> None:
    if DEFAULT_NB_STG_MSISDN_IMSI_OPERATOR_EXECUTED_PATH.exists():
        DEFAULT_NB_STG_MSISDN_IMSI_OPERATOR_EXECUTED_PATH.unlink()
    run_notebook(
        source_notebook=DEFAULT_NB_STG_MSISDN_IMSI_OPERATOR_NOTEBOOK_PATH,
        executed_notebook=DEFAULT_NB_STG_MSISDN_IMSI_OPERATOR_EXECUTED_PATH,
    )


def run_nb_stg_geo_intervals() -> None:
    if DEFAULT_NB_STG_GEO_INTERVALS_EXECUTED_PATH.exists():
        DEFAULT_NB_STG_GEO_INTERVALS_EXECUTED_PATH.unlink()
    run_notebook(
        source_notebook=DEFAULT_NB_STG_GEO_INTERVALS_NOTEBOOK_PATH,
        executed_notebook=DEFAULT_NB_STG_GEO_INTERVALS_EXECUTED_PATH,
    )


def run_nb_stg_person() -> None:
    if DEFAULT_NB_STG_PERSON_EXECUTED_PATH.exists():
        DEFAULT_NB_STG_PERSON_EXECUTED_PATH.unlink()
    run_notebook(
        source_notebook=DEFAULT_NB_STG_PERSON_NOTEBOOK_PATH,
        executed_notebook=DEFAULT_NB_STG_PERSON_EXECUTED_PATH,
    )
