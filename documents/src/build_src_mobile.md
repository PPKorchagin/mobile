...
**В scope задач:** привязка к BS и person, **два ЦОД** (`central` / `far-east`) по региону БС в момент события, локальное время (опционально TZ CSV), cross-mart шум, запись parquet. Абоненты: IMSI с MCC **250** (доля ~5% — роуминговые IMSI), MSISDN — РФ **7…** и **международные** E.164 (~18%).
