...
| 3 | Профиль полей, микс событий, gate `stg_contract.*` | JSON-метрики в лог `DQ_DDS_EVENT` |
| 4 | Сформировать `summary` | Счётчики checks и итоговый статус |

**Бизнес-назначение:** контроль качества дневного DDS-среза событий после [`build-dds-move-event`](../../dds/build_dds_move_event.md) перед потребителями geo-слоя.

**В scope задач:** рекурсивный обход каталога, чтение Parquet, фильтр по локальным суткам, проверки контракта `DDS_EVENT_FIELDS`, gate по идентификаторам и `location` (без PII в distribution).

---

## Параметры запуска

Вызов pipeline: `run_dq(report_date, event_dds_root)` ([`cli.py`](../../../src/mobile/cli.py) → `dq-dds-event`). **Оба параметра обязательны.**
...
uv run mobile dq-dds-event
uv run mobile dq-dds-event --report-date 2025-01-01
uv run mobile dq-dds-event --report-date 2025-01-01 --event-dds-path data/dds/event_dds
uv run mobile nb-dds-event
```

Логи: `data/logs/mobile.log` (тег `DQ_DDS_EVENT`). Метрики времени: `data/qa/command_timing.jsonl`, `command=dq-dds-event-{date}`. Визуализация: `nb-dds-event` → `data/notebooks/9_dds_event.executed.ipynb`.
...
| Слой хранения | `data/dds/event_dds/{YYYY-MM-DD}/{source_id}.parquet` |
| Формат | Parquet |
...
Контракт — [`event.json`](../../../src/mobile/schema/dds/event.json) → `fields`; см. [`build_dds_event.md`](../../dds/build_dds_event.md) → «Поля витрины».
...
| `source.{dc}.event_distribution` / `event_distribution` | ЦОД / день | Счётчики по коду `event` (10001…10004) | Sanity микса типов событий после `build-dds-event` |
...
| `source.{dc}.distribution.event_count_bucket` / `distribution.event_count_bucket` | ЦОД / день | Корзины `1` / `2-5` / `6-20` / `21+` | Контроль доли 5m-схлопывания из [`build-dds-event`](../../dds/build_dds_event.md) |
...
| `source.{dc}.null_rates` / `null_rates` | ЦОД / день | `null_rate_by_column` по `DDS_EVENT_FIELDS` | Полнота полей канонической витрины |
...
| `source.{dc}.event_count_valid` / `event_count_valid` | **failed** | Строки с `event_count` < 1 или не число; в метриках — `aggregated_rows`, `aggregated_share` (`event_count>1`) | Инвариант ETL: каждая строка — ≥1 событие; доля схлопнутых групп — контроль 5m-bucket из [`build-dds-event`](../../dds/build_dds_event.md) |
...
| `stg_contract.columns` | **failed** | Отсутствует колонка из `DDS_EVENT_FIELDS` | Минимальный контракт [`event.json`](../../../src/mobile/schema/dds/event.json) для потребителей DDS |

Обязательные поля (`DDS_EVENT_FIELDS`): `event_timestamp`, `imsi`, `imei`, `msisdn`, `location`, `event`, `event_name`, `event_count`.
...
| Схема | [`event.json`](../../../src/mobile/schema/dds/event.json) |
| DQ mobile (вход) | [`dq_src_mobile.md`](../src/dq_src_mobile.md) |

Сквозная цепочка: `build-src-mobile` → `build-dds-event` → `build-dds-move-event` → `dq-dds-event` → `nb-dds-event` → geo/downstream.
