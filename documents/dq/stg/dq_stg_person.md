...
**Витрина:** `stg_person` · **Команда:** `dq-stg-person` · **Режим:** read-only DQ (не изменяет данные, не падает при failed checks).

Референс: [`pipelines/dq/stg/person.py`](../../../src/mobile/pipelines/dq/stg/person.py). Сборка: [`build_stg_person.md`](../../stg/build_stg_person.md). Схема: [`person.json`](../../../src/mobile/schema/stg/person.json).
...
| # | Задача | Результат |
|---|--------|-----------|
| 1 | Найти месячный parquet `stg_person` за `report_date` | Путь `{YYYY-MM-01}.parquet` |
| 2 | Проверить контракт колонок и null-профиль | Логи `DQ_STG_PERSON` |
...
1. Сверка `person_id` с **прошлым** месяцем `stg_person` (стабильность между срезами).
...
| `_PERSON_COLUMNS` | имена полей из `STG_PERSON_FIELDS` ([`stg/person.py`](../../../src/mobile/pipelines/stg/person.py)) |
...
uv run mobile dq-stg-person
uv run mobile dq-stg-person --report-date 2025-01-15 \
...
Логи: `data/logs/mobile.log` (тег `DQ_STG_PERSON`). Метрики времени: `data/qa/command_timing.jsonl`, `command=dq-stg-person` или `dq-stg-person-{YYYY-MM-01}`. Визуализация: `nb-stg-person` → `data/notebooks/15_stg_person.executed.ipynb`.
...
| Свойство | Значение |
|----------|----------|
| Имя таблицы | `stg_person` — [`person.json`](../../../src/mobile/schema/stg/person.json) |
| Путь по умолчанию | `data/stg/person/{YYYY-MM-01}.parquet` |
| Формат | Parquet (`snappy`) |
| Гранулярность | Месячный срез (`report_date` = 1-е число месяца) |
| Контракт полей | `STG_PERSON_FIELDS` из [`pipelines/stg/person.py`](../../../src/mobile/pipelines/stg/person.py) |
...
Формат лога: `{"tag":"DQ_STG_PERSON","check":"...","status":"...","metrics":{...}}`.

| Check | Статус при сбое | Смысл | Обоснование |
|-------|-----------------|-------|-------------|
| `dataset_presence` | **failed** | Parquet за месяц не найден | Нет среза после [`build-stg-person`](../../stg/build_stg_person.md) |
| `dataset_basic` | **ok** | `row_count`, `column_count`, `distinct_person_id`, путь | Базовый объём для сравнения прогонов |
| `schema_columns` | **failed** / **warning** | `missing_columns` / `extra_columns` | Контракт совпадает с ETL и [`person.json`](../../../src/mobile/schema/stg/person.json) |
...
| DQ notebook | [`pipelines/nb/15_stg_person.ipynb`](../../../src/mobile/pipelines/nb/15_stg_person.ipynb) |
