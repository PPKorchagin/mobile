...
**Витрина:** `stg_bs` · **Команда:** `dq-stg-bs` · **Режим:** read-only DQ (процесс не падает при failed checks).

Референс: [`pipelines/dq/stg/bs.py`](../../../src/mobile/pipelines/dq/stg/bs.py). Сборка: [`build_stg_bs.md`](../../stg/build_stg_bs.md). Схема: [`bs.json`](../../../src/mobile/schema/stg/bs.json).
...
| # | Задача | Результат |
|---|--------|-----------|
| 1 | Прочитать parquet `stg_bs` | DataFrame витрины |
| 2 | Проверить схему, ключи CGI, SCD-интервалы, координаты, словари, WKT | JSON-метрики в лог `DQ_STG_BS` |
| 3 | Сформировать `summary` | Счётчики checks и итоговый статус |

**Бизнес-назначение:** контроль качества исторической витрины БС после [`build-stg-bs`](../../stg/build_stg_bs.md) перед [`build-stg-geo-all`](../../stg/build_stg_geo_all.md) и интервалами.

**В scope задач:** наличие файла, колонки `STG_BS_FIELDS`, профиль `nulls.*` / `cardinality.*`, ключ `(mcc,mnc,lac,cell_id)`, уникальность среза по `date_on`, порядок `date_on`/`date_off`, координаты, словари `bs_type` / `telecomstandard`, WKT `sector_wkt` и `mapinfo_wkt`.
...
Вызов pipeline: `run_dq(parquet_path)` ([`cli.py`](../../../src/mobile/cli.py) → `dq-stg-bs`).

| Переменная | Тип | Обязательность | Значение по умолчанию | Описание |
|------------|-----|----------------|----------------------|----------|
| `parquet_path` | path | Нет | `data/stg/bs.parquet` | Историческая витрина `stg_bs` (`stg_bs_output_path()`) |

**CLI:** `--stg-bs-path` — явный parquet; без флага — `stg_bs_output_path()` ([`project_paths.py`](../../../src/mobile/project_paths.py)).
...
uv run mobile dq-stg-bs --stg-bs-path data/stg/bs.parquet
uv run mobile nb-stg-bs
```

Логи: `data/logs/mobile.log` (тег `DQ_STG_BS`). Метрики времени: `data/qa/command_timing.jsonl`, `command=dq-stg-bs`. Визуализация: `nb-stg-bs` → `data/notebooks/10_stg_bs.executed.ipynb`.
...
| Свойство | Значение |
|----------|----------|
| Имя таблицы | `stg_bs` — [`bs.json`](../../../src/mobile/schema/stg/bs.json) → `table` |
| Путь по умолчанию | `data/stg/bs.parquet` |
| Формат | Parquet |
| Модель данных | SCD Type 2: интервалы `date_on` / `date_off` |
| Контракт полей | `STG_BS_FIELDS` из [`pipelines/stg/bs.py`](../../../src/mobile/pipelines/stg/bs.py) |

### Поля (контракт)

28 колонок — [`bs.json`](../../../src/mobile/schema/stg/bs.json) → `fields`. Ключевые для DQ:
...
|---|----------|------|------------|
| 1 | `stg_bs` | `data/stg/bs.parquet` | Единый исторический parquet после `build-stg-bs` |
...
2. Список ожидаемых колонок — `STG_BS_FIELDS`.
...
| Ошибка | Причина |
|--------|---------|
| `dataset_presence` **failed** | Нет `data/stg/bs.parquet` |
| `schema_columns` **failed** | Нет колонок из `STG_BS_FIELDS` |
...
Статусы: **ok** — метрика/успешный gate; **warning** / **failed** — отклонение. Формат лога: `{"tag":"DQ_STG_BS","check":"...","status":"...","metrics":{...}}`.
...
| `schema_columns` | **failed** | `missing_columns` vs `STG_BS_FIELDS` | Контракт совпадает с ETL [`build-stg-bs`](../../stg/build_stg_bs.md) и [`bs.json`](../../../src/mobile/schema/stg/bs.json) |
...
| DQ notebook | [`pipelines/nb/10_stg_bs.ipynb`](../../../src/mobile/pipelines/nb/10_stg_bs.ipynb) |
...
Сквозная цепочка: `build-src-bs` → `build-stg-bs` → `dq-stg-bs` → `nb-stg-bs` → `build-stg-geo-all` → downstream.
