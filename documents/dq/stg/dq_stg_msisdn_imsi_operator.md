...
**Витрина:** `stg_msisdn_imsi` · **Команда:** `dq-stg-msisdn-imsi-operator` · **Режим:** read-only DQ (не изменяет данные, не падает при failed checks).

Референс: [`pipelines/dq/stg/msisdn_imsi_operator.py`](../../../src/mobile/pipelines/dq/stg/msisdn_imsi_operator.py). Сборка: [`build_stg_msisdn_imsi_operator.md`](../../stg/build_stg_msisdn_imsi_operator.md). Схема: [`msisdn_imsi.json`](../../../src/mobile/schema/stg/msisdn_imsi.json).
...
| # | Задача | Результат |
|---|--------|-----------|
| 1 | Найти месячный parquet `stg_msisdn_imsi` за `report_date` | Путь `{YYYY-MM-01}.parquet` |
| 2 | Проверить контракт колонок и null-профиль | Логи `DQ_STG_MSISDN_IMSI_OPERATOR` |
| 3 | Проверить MSISDN/IMSI/`operator_id` и интервалы | Gate-статусы `ok/warning/failed` |
| 4 | Выдать `summary` | Счётчики checks |

**Бизнес-назначение:** контроль качества месячной витрины MSISDN↔IMSI↔`operator_id` после [`build-stg-msisdn-imsi-operator`](../../stg/build_stg_msisdn_imsi_operator.md) перед [`build-stg-person`](../../stg/build_stg_person.md) и [`build-stg-geo-intervals`](../../stg/build_stg_geo_intervals.md).

**В scope:** наличие файла, контракт `STG_MSISDN_IMSI_FIELDS`, null-профиль, порядок `valid_from`/`valid_to`, нормализация идентификаторов, согласованность `operator_id` с IMSI (MCC=250), дубликаты, пересечения и несклеенные сегменты по `(msisdn, operator_id, imsi)`.
...
Вызов: `run_dq(report_date, stg_msisdn_imsi_path)` ([`cli.py`](../../../src/mobile/cli.py) → `dq-stg-msisdn-imsi-operator`). **Оба параметра обязательны** при явном прогоне — pipeline не подставляет пути по умолчанию; их резолвит CLI-оркестратор или явный вызов.
...
| `stg_msisdn_imsi_path` | path | **Да** | Месячный parquet или каталог `data/stg/msisdn_imsi` (для каталога — файл `{YYYY-MM-01}.parquet`) |

### CLI

| Режим | Поведение |
|-------|-----------|
| Без флагов | Цикл `DEFAULT_SRC_START_DATE` … `DEFAULT_SRC_END_DATE` ([`cli_defaults.py`](../../../src/mobile/cli_defaults.py)); **один прогон на календарный месяц**, если `stg_msisdn_imsi_output_path(day)` существует; timed-run `dq-stg-msisdn-imsi-operator-{YYYY-MM-01}` |
| Оба явно | `--report-date` (любой день, например `2025-01-15` → месяц `2025-01-01`) и `--stg-msisdn-imsi-path` |
...
**Предусловие:** `uv run mobile build-stg-msisdn-imsi-operator` за дни месяца с `stg_geo_all`.

Локальный запуск:

```bash
uv run mobile build-stg-msisdn-imsi-operator
uv run mobile dq-stg-msisdn-imsi-operator
uv run mobile dq-stg-msisdn-imsi-operator --report-date 2025-01-15 \
  --stg-msisdn-imsi-path data/stg/msisdn_imsi/2025-01-01.parquet
uv run mobile dq-stg-msisdn-imsi-operator --report-date 2025-01-01 --stg-msisdn-imsi-path data/stg/msisdn_imsi
uv run mobile nb-stg-msisdn-imsi-operator
```

Логи: `data/logs/mobile.log` (тег `DQ_STG_MSISDN_IMSI_OPERATOR`). Метрики времени: `data/qa/command_timing.jsonl`, `command=dq-stg-msisdn-imsi-operator` или `dq-stg-msisdn-imsi-operator-{YYYY-MM-01}`. Визуализация: `nb-stg-msisdn-imsi-operator` → `data/notebooks/13_stg_msisdn_imsi_operator.executed.ipynb`.
...
| Путь по умолчанию | `data/stg/msisdn_imsi/{YYYY-MM-01}.parquet` |
| Формат | Parquet (`snappy`) |
| Гранулярность | Месячный файл, пополняется ежедневно из `stg_geo_all` |
| Контракт полей | `STG_MSISDN_IMSI_FIELDS` из [`pipelines/stg/msisdn_imsi.py`](../../../src/mobile/pipelines/stg/msisdn_imsi.py) |
...
|---|----------|------|------------|
| 1 | `stg_msisdn_imsi` | `data/stg/msisdn_imsi/{YYYY-MM-01}.parquet` | Месячные интервалы MSISDN↔IMSI↔operator после `build-stg-msisdn-imsi-operator` |
...
2. `_resolve_source_path(report_month, stg_msisdn_imsi_path)` — каталог → `{YYYY-MM-01}.parquet`, иначе файл как есть.
...
Формат лога: `{"tag":"DQ_STG_MSISDN_IMSI_OPERATOR","check":"...","status":"...","metrics":{...}}`.

| Check | Статус при сбое | Смысл | Обоснование |
|-------|-----------------|-------|-------------|
| `dataset_presence` | **failed** | Parquet за месяц не найден | Нет витрины после [`build-stg-msisdn-imsi-operator`](../../stg/build_stg_msisdn_imsi_operator.md) |
| `dataset_basic` | **ok** | объём, `distinct_msisdn`, `distinct_operator_id` | Базовый профиль месяца |
| `schema_columns` | **failed** | `missing_columns` | Контракт ETL / [`msisdn_imsi.json`](../../../src/mobile/schema/stg/msisdn_imsi.json) |
...
| DQ notebook | [`pipelines/nb/13_stg_msisdn_imsi_operator.ipynb`](../../../src/mobile/pipelines/nb/13_stg_msisdn_imsi_operator.ipynb) |
...
| Схема | [`msisdn_imsi.json`](../../../src/mobile/schema/stg/msisdn_imsi.json) |
| DQ IMEI | [`dq_stg_msisdn_imei.md`](./dq_stg_msisdn_imei.md) |

Сквозная цепочка: `build-stg-geo-all` → `build-stg-msisdn-imei` → `dq-stg-msisdn-imei` → **`build-stg-msisdn-imsi-operator`** → **`dq-stg-msisdn-imsi-operator`** → **`nb-stg-msisdn-imsi-operator`** → `build-stg-person` / `build-stg-geo-intervals` → downstream.
