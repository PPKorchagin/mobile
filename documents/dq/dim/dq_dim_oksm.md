...
| ------------------ | --------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `dataset_presence` | **failed**      | Parquet по `oksm_path` не найден; дальнейшие checks не выполняются                                              | Без файла витрины DQ и downstream (`build-fct-person`) не имеют входа |
...
| ------------------------ | --------------- | ------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| `numeric_code_integrity` | **failed**      | `numeric_code` не `^\d{3}$` или дубликаты; `invalid_numeric_code_count`, `duplicate_numeric_code_count` | `numeric_code` — ключ страны для `fct_person.citizenship`   |
