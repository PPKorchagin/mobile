...
| 4 | Синхронизация месячных `fct_msisdn_imsi` / `fct_msisdn_imei` из `stg_geo_all` по дням месяца | `{YYYY-MM-01}.parquet` для обеих binding-витрин |
...
| 8 | `person_id` из прошлого `fct_person` по `person_cluster_key` | Стабильный ID между месяцами |
| 9 | Запись `fct_person` | 1 строка на персону, поле `sim_count` |
...
- [`build-dim-tac`](../dim/build_dim_tac.md) → `data/dim/tac.parquet` (иначе M2M-фильтр пропускается с warning).
- [`build-dim-oksm`](../dim/build_dim_oksm.md) → `data/dim/oksm.parquet` (обязателен для `citizenship`).
- [`build-stg-geo-all`](../stg/build_stg_geo_all.md) по дням месяца (pipeline дополняет binding-витрины при `sync_bindings_from_geo=true`, по умолчанию).
...
| `fct_msisdn_imsi_path` | path | Нет | `data/fct/msisdn_imsi/{YYYY-MM-01}.parquet` | MSISDN↔IMSI за месяц |
| `fct_msisdn_imei_path` | path | Нет | `data/fct/msisdn_imei/{YYYY-MM-01}.parquet` | MSISDN↔IMEI за месяц |
...
| `output_path` | path | Нет | `data/fct/person/{YYYY-MM-01}.parquet` | Выход `fct_person` |
...
MNP и смена IMSI учитываются через интервалы в месячной `fct_msisdn_imsi` (наблюдения из `stg_geo_all` по дням месяца).
...
Binding-витрины можно собрать заранее по дням (`build-fct-msisdn-imei`, `build-fct-msisdn-imsi-operator`) или положиться на синхронизацию внутри `build-fct-person` из `stg_geo_all`.

Логи: `data/logs/mobile.log`. Метрики: `data/qa/command_timing.jsonl`, `command=build-fct-person`.
...
|---------|------|----------------|
| `fct_person` | `data/fct/person/{YYYY-MM-01}.parquet` | 1 строка на `person_id` (основная подписка + `sim_count`) |
...
| 2 | `fct_msisdn_imsi` | `data/fct/msisdn_imsi/{YYYY-MM-01}.parquet` | MSISDN↔IMSI + `operator_id`, MNP |
| 3 | `fct_msisdn_imei` | `data/fct/msisdn_imei/{YYYY-MM-01}.parquet` | MSISDN↔IMEI |
...
| 7 | `fct_person` (прошлый месяц) | `data/fct/person/{prev YYYY-MM-01}.parquet` | Стабильный `person_id` |
...
- [`build_dim_tac.md`](../dim/build_dim_tac.md)
- [`build_dim_oksm.md`](../dim/build_dim_oksm.md)
- [`build_fct_msisdn_imsi_operator.md`](./build_fct_msisdn_imsi_operator.md)
- [`build_fct_msisdn_imsi_operator.md`](./build_fct_msisdn_imsi_operator.md) (geo / IMSI)
- [`build_fct_msisdn_imei.md`](./build_fct_msisdn_imei.md)
...
4. Разрешение путей: `person_out`, месячные `fct_msisdn_imsi` / `fct_msisdn_imei`, списки excl.
5. Загрузка контракта полей из [`person.json`](../../src/mobile/schema/fct/person.json).
...
### Шаг 4. Витрина `fct_msisdn_imsi` (MNP из src_person)
...
4. **Binding-fill** на момент `binding_at` = конец последнего дня месяца (`_enrich_identifiers_from_bindings`):
   - для пустого `imsi` — lookup по `msisdn` в `fct_msisdn_imsi` (интервал содержит `at`);
