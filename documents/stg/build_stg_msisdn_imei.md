...
| 3 | Инкремент в месячный parquet | `data/stg/msisdn_imei/{YYYY-MM-01}.parquet` |

**Бизнес-назначение:** месячная привязка номера к устройству (смена IMEI при том же MSISDN) для дозаполнения в [`build-stg-geo-intervals`](./build_stg_geo_intervals.md) и рёбер графа в [`build-stg-person`](./build_stg_person.md).
...
Вызов: `run_build(report_date, stg_geo_all_path, output_path)` ([`cli.py`](../../src/mobile/cli.py) → `build-stg-msisdn-imei`). **Все три обязательны** — pipeline не подставляет пути по умолчанию; их резолвит CLI-оркестратор или явный вызов.
...
| `output_path` | path | **Да** | Месячный parquet `stg_msisdn_imei` |
...
| Режим | Поведение |
|-------|-----------|
| Без флагов | Цикл `DEFAULT_SRC_START_DATE` … `DEFAULT_SRC_END_DATE` ([`cli_defaults.py`](../../src/mobile/cli_defaults.py)); дни с существующим `stg_geo_all`; timed-run `build-stg-msisdn-imei-{YYYY-MM-DD}` с путями `data/stg/geo_all/{date}.parquet` → `data/stg/msisdn_imei/{YYYY-MM-01}.parquet` |
...
Логи: `data/logs/mobile.log` (строка `build-stg-msisdn-imei completed`). Метрики: `data/qa/command_timing.jsonl`, `command=build-stg-msisdn-imei` или `build-stg-msisdn-imei-{date}`.
...
Шаблон: `STG_MSISDN_IMEI_LAYOUT_TEMPLATE` в [`project_paths.py`](../../src/mobile/project_paths.py):

`data/stg/msisdn_imei/{YYYY-MM-01}.parquet`

### Поля витрины

Контракт — [`msisdn_imei.json`](../../src/mobile/schema/stg/msisdn_imei.json) → `fields`.
...
| Артефакт | Путь |
|----------|------|
| Схема витрины | [`src/mobile/schema/stg/msisdn_imei.json`](../../src/mobile/schema/stg/msisdn_imei.json) |
...
| IMSI + operator | [`build_stg_msisdn_imsi_operator.md`](./build_stg_msisdn_imsi_operator.md) |
| Person | [`build_stg_person.md`](./build_stg_person.md) |
| Geo-intervals | [`build_stg_geo_intervals.md`](./build_stg_geo_intervals.md) |
| DQ | [`dq_stg_msisdn_imei.md`](../dq/stg/dq_stg_msisdn_imei.md) |

Сквозная цепочка: `build-stg-geo-all` → **`build-stg-msisdn-imei`** → **`dq-stg-msisdn-imei`** → **`nb-stg-msisdn-imei`** → downstream.
