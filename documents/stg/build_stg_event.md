...
2. Включить `build-stg-event` в сквозную цепочку, если потребуется оркестратор.
...
1. Сортировка: `imsi`, `event_timestamp`, `event_name` (стабильный порядок для группировки).
2. Разделение на потоки по валидности CGI (`mcc`, `mnc`, `lac`, `cell` — числовые и в допустимых диапазонах).

**Поток A — валидный CGI:**

1. `event_timestamp` → 5-минутный bucket (`floor` к 300 сек).
2. Группировка: `(imsi, event_name, mcc, mnc, lac, cell, bucket)`.
3. Внутри группы: если разрыв между соседними `event_timestamp` > **300 сек** — новая выходная строка.
4. Агрегат группы: `event_count = len(group)`; `start`/`end` = min/max timestamp группы.
5. Поля `location`, координаты — из представительной строки группы (первая/last по политике в коде).

**Поток B — невалидный CGI:**

1. **Без** 5m-схлопывания: каждая исходная строка → одна выходная.
2. `event_count = 1` всегда.
3. Сохраняются для downstream-аналитики «битых» событий.

4. `pd.concat` потоков A и B; финальная сортировка перед записью.
