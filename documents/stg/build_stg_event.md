...
**Витрина:** `stg_event` · **Команда:** `build-stg-event` · **Режим:** сборка дневного Parquet из mobile-витрин одного ЦОД.
...
**Константы ETL в коде** (на вход job **не передаются**): `STG_EVENT_FIELDS`, `_MART_READ_COLUMNS`, `_COMPRESS_GAP_SECONDS` — см. [`event.py`](../../src/mobile/pipelines/stg/event.py).
...
| `--report-date` + `--dc` | Один прогон: 4 корня из `mobile_mart_paths`, выход `data/stg/event/{YYYY}/{MM}/{DD}/{dc}/events.parquet` |
...
uv run mobile build-stg-event --report-date 2025-01-01 \
  ...
  --output-path data/stg/event/2025/01/01/central/events.parquet
```

Логи: `data/logs/mobile.log` (`build-stg-event report_date=… path=…`). Метрики: `data/qa/command_timing.jsonl`, `command=build-stg-event-{dc}-{date}`.
...
1. `_transform_mart_frame` → канон `stg_event`.
...
| DDS перенос | [`build_move_event.md`](./build_move_event.md) |
